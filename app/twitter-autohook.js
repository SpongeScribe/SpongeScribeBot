    ...
    try {
      ...
      webhook.on('event', async event => {
        try {
          ...
          fs.appendFile('./data/logs/event.json', JSON.stringify(event) + "\n", 'utf8', (err, fd) => {
            if (err) throw err;
          });
          ...
          if (event.tweet_create_events) {
            fs.appendFile('./data/logs/tweet.json', JSON.stringify(event) + "\n", 'utf8', (err, fd) => {
              if (err) throw err;
            });
            const user_id = event.for_user_id
            const tweets = event.tweet_create_events.map(async (tweet) => {
              if (tweet.user.id_str === user_id) {
                fs.appendFile('./data/logs/tweet.self.json', JSON.stringify(tweet) + "\n", 'utf8', (err, fd) => {
                  if (err) throw err;
                });
              } else {
                fs.appendFile('./data/logs/tweet.other.json', JSON.stringify(tweet) + "\n", 'utf8', (err, fd) => {
                  if (err) throw err;
                });
                tweetThreadReply(
                  client,
            ...
            Promise.all(tweets).then((completed) => console.log(JSON.stringify(completed)));
          }
        } catch (e) {
