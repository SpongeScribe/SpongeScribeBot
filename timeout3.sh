...
# { sed /"$keywords"/q; kill $!; } < <(exec timeout 1m tail -Fn 0 $log_file)
    ...
    # {
      # (
        # docker logs --details --follow --timestamps $DOCKER_CONTAINER_ID
        # >&3 3>&-; echo "$?") |
    ...
    # }
    # timeout4 --foreground 30 sh -c "$(docker logs --details --follow --timestamps $DOCKER_CONTAINER_ID) ; exit"
